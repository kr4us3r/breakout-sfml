...
// ======================================================================
// Helper: draw a heart shape from a ConvexShape (two triangles + rect)
// ======================================================================
static sf::ConvexShape makeHeart(float cx, float cy, float size, sf::Color color) {
    sf::ConvexShape heart;
    heart.setPointCount(6);
    // Rough heart silhouette
    heart.setPoint(0, {cx - size * 0.5f, cy - size * 0.1f});
    heart.setPoint(1, {cx - size * 0.25f, cy - size * 0.5f});
    heart.setPoint(2, {cx, cy - size * 0.2f});
    heart.setPoint(3, {cx + size * 0.25f, cy - size * 0.5f});
    heart.setPoint(4, {cx + size * 0.5f, cy - size * 0.1f});
    heart.setPoint(5, {cx, cy + size * 0.45f});
    heart.setFillColor(color);
    return heart;
}

// ======================================================================
// Helper: draw a classic game-style heart using a parametric heart curve
...
static void drawHeart(sf::RenderWindow& win, float cx, float cy, float size, sf::Color color) {
    ...
    float scale = size / 32.f;  // normalise: curve spans roughly -16..16 horizontally
    ...
    for (int i = 0; i < points; ++i) {
        ...
        // Flip Y (screen coords go down) and centre
...
Game::Game() : window(sf::VideoMode({window_width, window_height}), "breakout",
    ...
    // Load a system font (Arial on Windows)
    if (!font.openFromFile("C:/Windows/Fonts/arial.ttf")) {
        // fallback: try Times New Roman for a more serif look
...
void Game::render() {
    ...
    if (state == State::DEAD) {
        // Draw frozen game behind death screen
...
void Game::renderDeathScreen() {
    ...
    // "YOU DIED" — Dark Souls style: serif, large, dark red
    sf::Text died(font);
    died.setString("YOU DIED");
    died.setCharacterSize(120);
    died.setFillColor(sf::Color(120, 20, 20));
...
void Game::moveBall(float /*dt*/) {
    ...
    // CCD sub-stepping
    for (unsigned i = 0; i < ccd_substeps; ++i) {
        sf::Vector2f step = ball.velocity / static_cast<float>(ccd_substeps);
        ball.displace(step);
        handleWallCollision();
...
// ----------------------------------------------------------------------
// Spawn bricks with gaps and jitter
// ----------------------------------------------------------------------
void Game::spawnBricks() {
    ...
    for (unsigned y = 0; y < num_bricks_y; ++y) {
        for (unsigned x = 0; x < num_bricks_x; ++x) {
            // ~12% skip for gaps
            if (std::rand() % 100 < 12) continue;

            float jitter_x = static_cast<float>(std::rand() % 7 - 3);
            float jitter_y = static_cast<float>(std::rand() % 7 - 3);

            bricks.emplace_back(Brick(
                {offset_x + static_cast<float>(x) * (brick_width + brick_gap) + jitter_x,
                 offset_y + static_cast<float>(y) * (brick_height + brick_gap) + jitter_y},
...
void Game::run() {
    ...
    while (window.isOpen()) {
                    ...
                    if (state == State::MENU) {
                        ...
                        if (!ball_launched) {
                            ...
                            if (platform.charged) {
                                ...
                                platform.charge();
                                particles.emit(ball.getPosition(), {0.f, -50.f}, 12,
                                               sf::Color(255, 220, 80), 0,
                                               50, 200, 1.f, 3.f, 0.2f, 0.5f);
                            }
                        }
                    }
                }
        ...
        if (state == State::PLAYING) {
            ...
            // Ball follows platform while waiting
            if (!ball_launched) {
                sf::Vector2f plat_pos = platform.getPosition();
                ball.setPosition({plat_pos.x + static_ball_x_displacement, ball_y_initial});
            }
...
void Game::handleWallCollision() {
    ...
    if (ball_pos.y - ball_radius <= 0) {
        ...
        particles.emit(ball_pos, {0.f, 0.f}, 6, sf::Color(180, 200, 255), 0,
                       40, 150, 1.f, 2.5f, 0.2f, 0.4f);
    }
    ...
    if (ball_pos.y - ball_radius > static_cast<float>(window_height)) {
        // Emit death particles at the exact position where ball was lost
        particles.emit(ball_pos, {0.f, -200.f}, 30, sf::Color(255, 100, 100), 0,
                       100, 400, 2.f, 5.f, 0.4f, 0.9f);
...
void Game::handlePlatformCollision() {
    ...
    if (platform.charged) {
        ...
        particles.emit(ball_pos, {0.f, 0.f}, 20, sf::Color(255, 220, 80), 0,
                       100, 350, 2.f, 5.f, 0.3f, 0.8f);
        platform.discharge();
    } else {
        particles.emit(ball_pos, {0.f, 0.f}, 6, sf::Color(100, 180, 255), 0,
                       50, 150, 1.f, 3.f, 0.2f, 0.4f);
    }
}
...
void Game::handleBrickCollision() {
    ...
    for (Brick& brick : bricks) {
        ...
        if (dist_sq <= ball_radius * ball_radius) {
            ...
            unsigned pcount = brick.destroyed ? 25 : 8;
            particles.emit(ball_pos, {0.f, 0.f}, pcount, pcolor, 0,
                           80, 280, 1.5f, 4.f, 0.3f, 0.7f);
